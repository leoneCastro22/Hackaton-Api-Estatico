                ...
                HackatonStefaniniService.listar(vm.urlEndereco).then(
                    function (responseEndereco) {
                        if (responseEndereco.data !== undefined)
                            ...
                            vm.listaEnderecoMostrar = [];
                            var max = vm.listaEndereco.length > vm.qdePorPagina ? vm.qdePorPagina : vm.listaEndereco.length;

                            vm.qdePaginacao = new Array(vm.listaEndereco.length % vm.qdePorPagina === 0 ? vm.listaEndereco.length / vm.qdePorPagina : parseInt(vm.listaEndereco.length / vm.qdePorPagina) + 1);
                            vm.currentPage = 1;

                        for (var count = 0; count < max; count++) {
                            vm.listaEnderecoMostrar.push(vm.listaEndereco[count]);
                            vm.ultimoIndex++;
                        }

                        vm.listaEnderecoMostrar.sort(function (c, d) {
                        return c.id - d.id;
                });
                    }
                );
